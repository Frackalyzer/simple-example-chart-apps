...
def update_graph(selected, _remove, _n_clicks, x_value, y_value, text):
    ...
    for value in selected:
        trace.append(go.Scatter(x=df["Months"], y=df[value], mode="lines+markers",
                                ...
                                name=dropdown[value]))
    layout_go = go.Layout(colorway=["#EF533B", "#EF963B", "#287D95", "#2CB154", "#8C299D", "#8DD936"],
                          title="Temperature Over the Months", yaxis={"title": f"Temperature (degrees F)"},
                          xaxis={"title": "Months"})
    figure = {"data": trace, "layout": layout_go}
    if ctx.triggered[0]['prop_id'].split('.')[0] == 'submit-button':
        layout_go.update({"annotations": [
